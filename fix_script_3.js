const fs = require('fs');
const content = fs.readFileSync('src/pages/member/blok.tsx', 'utf8');

const marker = '{/* ── Trio Game Dialog ── */}';
const index = content.indexOf(marker);

if (index !== -1) {
    const cleanContent = content.substring(0, index);
    
    const newEnd = `                {/* ── Trio Game Dialog ── */}
                            ...
                            <div className="space-y-4">
                                ...
                                {trioRecords.map((record, index) => {
                                    ...
                                    const accent = index === 0
                                        ? { card: "border-amber-300 bg-gradient-to-br from-amber-50 to-yellow-100", badge: "bg-amber-500 text-white", total: "text-amber-600" }
                                        : index === 1
                                            ? { card: "border-slate-300 bg-gradient-to-br from-slate-50 to-slate-100", badge: "bg-slate-500 text-white", total: "text-slate-700" }
                                            : index === 2
                                                ? { card: "border-orange-300 bg-gradient-to-br from-orange-50 to-amber-100", badge: "bg-orange-500 text-white", total: "text-orange-700" }
                                                : { card: "border-purple-100 bg-white", badge: "bg-purple-100 text-purple-700", total: "text-purple-700" };

                                    return (
                                        <motion.div key={record.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} className={cn("rounded-3xl border p-3 sm:p-4 shadow-sm", accent.card)}>
                                            <div className="flex items-start justify-between gap-3">
                                                <div className="min-w-0">
                                                    <div className={cn("inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-bold", accent.badge)}>
                                                        <span>#{index + 1}</span>
                                                        {index === 0 ? <Crown className="h-3.5 w-3.5" /> : <Star className="h-3.5 w-3.5 fill-current" />}
                                                    </div>
                                                    <p className="mt-2 text-xs font-medium text-slate-500">
                                                        {record.include_handicap ? "Skor ahli termasuk handicap" : "Skor ahli tanpa handicap"}
                                                    </p>
                                                </div>
                                                {index < 3 && <Trophy className={cn("h-7 w-7 flex-shrink-0", index === 0 ? "text-amber-500" : index === 1 ? "text-slate-400" : "text-orange-500")} />}
                                            </div>

                                            <div className="mt-4 space-y-2.5">
                                                {players.map((item, playerIndex) => (
                                                    <div key={item.id} className="flex items-center gap-3 rounded-2xl border border-white/70 bg-white/80 px-3 py-2.5 shadow-sm">
                                                        <div className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full bg-slate-100 text-xs font-bold text-slate-600">{playerIndex + 1}</div>
                                                        {item.player?.avatar_url ? (
                                                            <Image src={item.player.avatar_url} alt={item.player.username} width={40} height={40} className="h-10 w-10 flex-shrink-0 rounded-full object-cover border-2 border-white" unoptimized loading="lazy" />
                                                        ) : (
                                                            <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full border-2 border-white bg-purple-100 font-bold text-purple-700 shadow-sm">{item.player?.username?.[0]?.toUpperCase() || "?"}</div>
                                                        )}
                                                        <div className="min-w-0 flex-1">
                                                            <Link href={\`/member/profile?id=\${item.id}\`} className="block truncate text-sm font-bold text-purple-900 hover:text-purple-700">@{item.player?.username}</Link>
                                                            <p className="text-[11px] text-slate-500">{record.include_handicap ? \`Handicap +\${item.handicap}\` : "Tanpa handicap"}</p>
                                                        </div>
                                                        <div className={cn("min-w-[76px] rounded-full px-3 py-1.5 text-center text-lg font-black", accent.total)}>{item.score}</div>
                                                    </div>
                                                ))}
                                            </div>

                                            <div className="mt-4 flex justify-end">
                                                <Button type="button" variant="outline" size="sm" onClick={() => void handleShareTrio(record, index + 1)} className="h-9 rounded-full border-white/70 bg-white/85 px-4 text-xs font-semibold text-purple-700 shadow-sm hover:bg-white">
                                                    <Share2 className="mr-1.5 h-4 w-4" /> Kongsi
                                                </Button>
                                            </div>
                                        </motion.div>
                                    );
...
`;
    ...
    fs.writeFileSync('src/pages/member/blok.tsx', cleanContent + newEnd);
    console.log("Success! Safely rebuilt blok.tsx.");
} else {
