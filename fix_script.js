...
try {
    // 1. Get the file content from the stable commit 56a10e4
    let content = execSync('git show 56a10e4:src/pages/member/blok.tsx').toString();

    // 2. Fix the Trio inside Double issue
    content = content.replace(
        /onClick=\{\(\) => void handleShareTrio\(record, index \+ 1\)\}/g,
        'onClick={() => void handleShareDouble(record, index + 1)}'
    );

    // 3. Add imports for Share & Close buttons + html2canvas
    content = content.replace(
    ...
    // 4. Inject handleShareMenVsWomen state and function
    const menVsWomenCode = `
...
`;
    ...
    // 5. Replace the Men vs Women Dialog with the new one containing the buttons
    const oldDialogRegex = /\{\/\* ── Men vs Women Dialog ── \*\/\}[\s\S]*?(?=\{\/\* ── Trio Game Dialog ── \*\/\}|<\/MemberLayout>)/;
    
    const newDialog = `{/* ── Men vs Women Dialog ── */}
...
`;
    ...
    console.log("Successfully rebuilt blok.tsx with Men vs Women dialog and buttons!");
} catch (e) {
    console.error(e);
}