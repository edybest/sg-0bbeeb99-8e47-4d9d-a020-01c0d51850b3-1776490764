-- Batch 4: members, mini_blok, mini_blok_collaborators, mini_blok_players
...
CREATE POLICY "Admins can manage members"
  ...
  FOR ALL
  TO authenticated
  USING (EXISTS (SELECT 1 FROM members m WHERE m.user_id = (SELECT auth.uid()) AND m.is_admin = true))
  WITH CHECK (EXISTS (SELECT 1 FROM members m WHERE m.user_id = (SELECT auth.uid()) AND m.is_admin = true));
