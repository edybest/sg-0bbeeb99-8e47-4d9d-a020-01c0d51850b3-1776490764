...
export default function BlokPage() {
    ...
    // FIX #3: Use a ref instead of state for previousLeaderboard so loadLeaderboard
    ...
    const loadGames = async (showToast = false) => {
        try {
            ...
            // FIX #6: Read query values directly from router inside the function.
            // The effect now also depends on router.query primitives (gameId, date)
            // so this closure always has the latest values when the effect re-runs.
    ...
    const loadLeaderboard = async (gameId: string) => {
        ...
        `
            ...
            // FIX #3 (continued): Read from the ref so we always compare against the
            // most recent snapshot regardless of when this closure was created.
            if (previousLeaderboardRef.current.length > 0) {
                const changedIds = new Set < string > ();
                const notifications: string[] = [];

                nextBase.forEach((newEntry) => {
                    const oldEntry = previousLeaderboardRef.current.find((old) => old.id === newEntry.id);
                    if (!oldEntry) return;
            ...
            // FIX #3 (continued): Write to the ref directly — no re-render needed.
    ...
    // FIX #6: Depend on the individual query primitives (gameId, date) rather than
    // the whole router object, so the effect re-runs when the URL params change
    // after client-side navigation without needing retryCount to bump.
    ...
    useEffect(() => {
        void loadGames();
    }, [retryCount, queryGameId, queryDate]); // eslint-disable-line react-hooks/exhaustive-deps

    // FIX #4: Add missing `void` to suppress the unhandled-promise warning.
    useEffect(() => {
        if (selectedGame) {
    ...
    const handleReaction = async (playerId: string, event: React.MouseEvent<HTMLButtonElement>) => {
        ...
        try {
            // FIX #2: Capture playerEntry BEFORE the optimistic state update so the
            // Supabase write uses the correct current count, not a stale closure value.
            ...
            if (currentUser.user_id) {
                ...
                // Update the game_players actual table using the captured pre-update count
                await supabase.
                    from('game_players').
    ...
    return (
        <MemberLayout>
          ...
          /* FIX #1: Define the 8 particle keyframes that were missing.
             Each travels in a different direction (N, NE, E, SE, S, SW, W, NW). */
        ...
        `}</style>
                ...
                {/* FIX #1 (continued): Each particle now references a keyframe that actually
            exists (particle-0 through particle-7), so they will animate correctly. */}
