...
export default function BlokPage() {
    ...
    const handleShareMenVsWomen = async () => {
        if (!menVsWomenRef.current || !menVsWomenData || isCapturingScreenshot) return;

        const element = menVsWomenRef.current;
        ...
        const fileName = `men-vs-women-${selectedGame || "game"}.png`;
        const selectedGameName =
            games.find((g) => g.id === selectedGame)?.game_name || "Game";
        ...
        if (navigator.share && navigator.canShare) {
            ...
            if (navigator.canShare({ files: [file] })) {
                await navigator.share({
                    title: "Men vs Women",
                    text: `Keputusan Men vs Women - ${selectedGameName}`,
                    files: [file],
                });
            } else {
        ...
        allNodes.forEach((node) => {
            const prev = previousStyles.get(node);
            if (!prev) return;

            node.style.animation = prev.animation;
            node.style.transition = prev.transition;
            node.style.transform = prev.transform;
            node.style.backdropFilter = prev.backdropFilter;
            node.style.filter = prev.filter;
        });
    } catch (error) {
        console.error("Gagal kongsi screenshot Men vs Women:", error);
        alert("Gagal menghasilkan screenshot. Cuba sekali lagi.");
    } finally {
                                                ...
                                                filteredLeaderboard.map((player, index) => {
                                                    ...
                                                    return (
                                                        <tr key={player.id} className={`border-b border-sky-100 transition-colors group ${rowBg}`}>
                                                            ...
                                                            <td
                                                                className={`sticky ${STICKY_LEFT.overall} z-10 ${isTop3 ? "bg-amber-100/80" : "bg-sky-100/80"
            ...
            <Dialog open={isMenVsWomenDialogOpen} onOpenChange={setIsMenVsWomenDialogOpen}>
                ...
                >
                    <div
                        ref={menVsWomenRef}
                        data-men-vs-women-capture="true"
                        className={`bg-slate-50 ${isCapturingScreenshot ? "overflow-visible screenshot-mode" : ""}`}
                    >
                        <div
                            className={`bg-gradient-to-r from-violet-600 to-indigo-600 px-4 py-4 sm:px-5 sm:py-5 text-white flex items-start sm:items-center justify-between gap-3 shadow-md ${isCapturingScreenshot ? "relative" : "sticky top-0 z-20"
                                }`}
                        >
